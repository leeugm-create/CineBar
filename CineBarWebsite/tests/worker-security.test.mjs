...
test("adds the audited security headers to dynamic and static HTTPS responses", async () => {
  for (const path of ["/", "/health", "/appcast.xml", "/og.png"]) {
    ...
    assert.equal(
      ...
      "default-src 'self'; base-uri 'self'; form-action 'self' https://www.paypal.com; frame-ancestors 'none'; img-src 'self' data: blob: https://image.tmdb.org https://moovie.c2v2.com; object-src 'none'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; connect-src 'self' https://api.cinebar.cc https://share.cinebar.cc https: blob:; media-src 'self' https: blob: data:; font-src 'self' data:; upgrade-insecure-requests",
      path,
    );
  }
});
